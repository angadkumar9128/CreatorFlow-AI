...
async function uploadGeminiFile(file, apiKey, onProgress) {
...
async function askGemini(fileRef, apiKey, prompt) {
  const key = cleanKey(apiKey);
...
function normalizeClip(clip, duration) {
  ...
  return {
    ...
    title: String(clip?.title || "AI selected moment"),
  };
}
...
export async function analyzeVideoForCreator(file, apiKey, duration, options = {}) {
  ...
  const fileRef = await uploadGeminiFile(file, apiKey, options.onProgress);
...
Select up to ${maxClips} strong, self-contained moments. Each clip should normally be about ${targetLength} seconds, but choose the natural start/end needed for a complete thought. Do not overlap clips. Prefer hooks, surprising facts, emotional moments, useful explanations, stories, questions, punchlines, strong opinions, or clear conclusions. Avoid long silence, greetings, repeated filler, and incomplete sentences.
...
{
  "clips": [
    {"start": 0, "end": 42, "score": 92, "title": "Short title", "reason": "Why this moment works"}
  ],
  "captions": [
...
- Keep clips at least 10 seconds and at most 180 seconds.
- Captions must stay inside 0..${duration}.
- Do not output markdown or commentary.
`;

  options.onProgress?.({ stage: "analyzing", ratio: 0.5 });
  const raw = await askGemini(fileRef, apiKey, prompt);
