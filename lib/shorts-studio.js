...
export function createAIShorts(sourceDuration, clips = []) {
  ...
  return clips
    .map((clip, i) => {
      const start = clamp(Number(clip.start) || 0, 0, Math.max(0, total - MIN_EDIT_SECONDS));
      const end = clamp(Number(clip.end) || start + DEFAULT_SHORT_SECONDS, start + MIN_EDIT_SECONDS, total);
