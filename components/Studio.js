...
const CREATOR_STYLES = [
  { id: "viral", label: "Viral", tone: "punchy", image: "vibrant", theme: "bold", mood: "energetic" },
  { id: "educational", label: "Educational", tone: "authority", image: "editorial", theme: "cool", mood: "balanced" },
  { id: "funny", label: "Funny", tone: "funny", image: "illustrated", theme: "bold", mood: "energetic" },
  { id: "premium", label: "Premium", tone: "authority", image: "editorial", theme: "warm", mood: "calm" },
  { id: "cinematic", label: "Cinematic", tone: "calm", image: "cinematic", theme: "warm", mood: "calm" },
  { id: "fast", label: "Fast", tone: "punchy", image: "vibrant", theme: "bold", mood: "energetic" },
  { id: "creative", label: "Creative", tone: "punchy", image: "illustrated", theme: "cool", mood: "balanced" },
  { id: "emotional", label: "Emotional", tone: "calm", image: "photoreal", theme: "warm", mood: "calm" },
  { id: "news", label: "News", tone: "authority", image: "editorial", theme: "mono", mood: "balanced" },
  { id: "auto", label: "Auto", tone: "punchy", image: "photoreal", theme: "bold", mood: null },
];
...
export default function Studio() {
  ...
  // Holds the look and the score together, so a re-render after a caption edit
  // produces the identical reel instead of a brand new one.
  const [renderSeed, setRenderSeed] = useState(newSeed);
  // isMobile/isIOS read navigator, which does not exist during server render.
  // Gating on mount keeps the first client paint identical to the server's.
  ...
  const nicheId = category === "custom" ? customCategory.trim().toLowerCase() : category;
  ...
  async function composeOutput(workingPlan, images, seedOverride) {
    ...
    if (isReel) {
      ...
      const state = {
        ...
        // Everything below drives the new motion and score engines.
        niche: nicheId,
        seed: seedOverride ?? renderSeed,
        mood: selectedStyle.mood || undefined,
        audio: {
          enabled: settings.soundEnabled,
          volume: Math.min(1, Math.max(0.15, Number(settings.soundVolume) || 0.85)),
          genre: settings.musicGenre || undefined,
        },
      };

      // recordReel sizes the canvas itself, because phones need 720x1280.
      const result = await recordReel(canvas, state, setRenderPct);
      ...
      setOutput({ url, kind: "image", ext: "jpg", mimeType: "image/jpeg", size: blob.size });
    }

    settle("render", "done");
  }

  async function produce({ reusePlan = null, reuseShots = null, freshLook = false } = {}) {
    if (!canGenerate) {
      setError("Add a custom category before generating.");
      return;
    }

    const seedForRun = freshLook || !reusePlan ? newSeed() : renderSeed;
    if (seedForRun !== renderSeed) setRenderSeed(seedForRun);
    ...
    try {
      ...
      await composeOutput(workingPlan, images, seedForRun);
      remember({
        id: crypto.randomUUID(),
  ...
  async function regenerateScene(index) {
    ...
    try {
      ...
      setShots(nextShots);
      settle(`img-${index}`, "done");
      await composeOutput(plan, nextShots, renderSeed);
    } catch (err) {
      settle(`img-${index}`, "fail");
      setError(err.message);
    } finally {
  ...
  async function applyEdits() {
    ...
    clearOutput();
    try {
      await composeOutput(plan, shots, renderSeed);
    } catch (err) {
      setError(err.message);
    } finally {
      setBusy(false);
  ...
  /**
   * On iOS an anchor with `download` does nothing useful, so this routes
   * through the OS share sheet where "Save Video" writes to Photos.
   */
  async function download() {
    ...
    try {
      ...
      setSaveState({
        ...
        message:
          result.note ||
          (result.method === "download" ? `Saved as ${result.filename}` : `Opened ${result.filename}`),
      });
    } catch (err) {
      setSaveState({ state: "fail", message: err.message });
    }
  }
      ...
      const { blob: mp4, usedFfmpeg } = await toInstagramMp4(blobRef.current, {
        ...
        onProgress: (info) =>
          setConverting({
            stage: info.stage,
            ratio: info.ratio ?? 0,
            message: info.detail || (info.stage === "converting" ? "Converting" : "Working"),
          }),
      });
          ...
          <div className="generate-grid">
              ...
              <section className="panel">
                ...
                <div className="form-grid">
                  ...
                  <label>
                    ...
                    >
                      <option value="fast">Fast — cheapest, highest daily volume</option>
                      <option value="balanced">Balanced — sharper, moderate volume</option>
                      <option value="best">Best — sharpest Cloudflare has, few per day</option>
                    </select>
                  </label>
                  {isReel && (
                    <label>
                  ...
                  {isReel && (
                    <label>
                      ...
                      <select
                        ...
                        onChange={(event) => updateSetting("musicGenre", event.target.value)}
                      >
                        <option value="">Match my category</option>
                ...
                {isReel && (
                  <div className="form-grid">
                    <label>
                      Music volume {Math.round((Number(settings.soundVolume) || 0.85) * 100)}%
                      <input
                        type="range"
                        min="0"
                        max="1"
                        ...
                        onChange={(event) => updateSetting("soundVolume", Number(event.target.value))}
                      />
                    </label>
                    <label className="checkbox-label">
                      ...
                      />
                      Include a generated soundtrack
                    </label>
                  </div>
                )}
              ...
              <section className="panel action-panel">
                ...
                <div className="button-row">
                  ...
                  <button
                    ...
                    onClick={() => produce({ reusePlan: plan, reuseShots: shots, freshLook: true })}
                  >
                    New Look and Sound
                  </button>
                ...
                {busy && isReel && (
                  <p className="muted">
                    Keep this screen open. Rendering runs in real time and pauses if the app goes to the background.
                  </p>
                )}
                {error && (
            ...
            <aside className="preview-rail">
              ...
              <div className="preview-actions">
                <button className="primary-button" disabled={!output || saveState?.state === "working"} onClick={download}>
                  {saveState?.state === "working" ? "Preparing" : mounted && isMobile() ? "Save to device" : "Download"}
                </button>
              ...
              {output?.kind === "video" && (
                <OutputFacts
                  ...
                  onConvert={convertForInstagram}
                />
              )}

              {saveState && (
                <p className={saveState.state === "fail" ? "alert" : "preview-note"}>{saveState.message}</p>
              )}

              <p className="preview-note">
                {mounted && isIOS()
                  ? "Saving opens the iOS share sheet. Choose Save Video so it lands in Photos, then post from Instagram."
...
function OutputFacts({ output, check, converting, onConvert }) {
  const seconds = output.durationSeconds ? `${output.durationSeconds.toFixed(1)}s` : "unknown";
  const megabytes = output.size ? `${(output.size / 1024 / 1024).toFixed(1)} MB` : "";

  return (
    <div className="panel output-facts">
      ...
      <div className="fact-row">
        <span>Format</span>
        <strong>
          {output.ext.toUpperCase()} {output.width}x{output.height} {megabytes}
        </strong>
      </div>
      {output.music && (
        <div className="fact-row">
          ...
          <strong>
            {output.music.label}, {output.music.bpm} bpm
          </strong>
        </div>
      )}
      {output.droppedFrames > 30 && (
        <p className="muted">
          {output.droppedFrames} frames were dropped while encoding. The reel still plays, but closing other
          apps or tabs before rendering will make it smoother.
        </p>
      )}

      {check && !check.ok && (
        <div className="alert">
          <strong>Instagram will reject this as it is.</strong>
          {check.problems.join(" ")}
        </div>
      )}
