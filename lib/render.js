...
import { fixContainerDuration } from "./duration-fix.js";
...
function pickMimeType(withAudio) {
  ...
  // Android Chrome can report a MediaRecorder MP4 with mismatched track timing
  // when canvas video and WebAudio are captured together. Keep the known-good
  // WebM intermediate on Android; remux/encode only when the user downloads.
  ...
  const candidates = android ? [...webm, ...mp4] : [...mp4, ...webm];
...
export async function recordReel(canvas, state, onProgress) {
  ...
  return new Promise((resolve, reject) => {
    ...
    recorder.onstop = async () => {
      ...
      // stop reporting 0:00.
      blob = await fixContainerDuration(blob, Math.round(elapsedMs));
