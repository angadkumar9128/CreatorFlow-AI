...
export async function saveFile(blob, baseName) {
  ...
  // IMPORTANT: do this before creating the File/download URL. A browser
  // preview can play a fragmented MP4 correctly while the actual downloaded
  // file still contains unusable duration metadata. Remuxing with +faststart
  // creates a normal MP4 whose moov atom contains the real duration.
  if (/mp4|m4v|quicktime/i.test(blob.type || "")) {
    finalBlob = await toInstagramMp4(blob);
  }
