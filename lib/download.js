...
export async function saveFile(blob, baseName, options = {}) {
  ...
  if (/video\/(mp4|webm|matroska)|mp4|m4v|quicktime/i.test(blob.type || "")) {
    try {
      ...
      onProgress?.({
        stage: "fallback",
        detail: "Converter unavailable; downloading the rendered video directly.",
      });
      finalBlob = blob;
    }
  }
  ...
  try {
    ...
    if ("download" in anchor) {
      ...
      return {
        ...
        note: conversionFailed
          ? "Downloaded the rendered video directly. MP4 duration conversion was unavailable."
          : undefined,
      };
    }
    ...
    return {
      ...
      filename,
      note: conversionFailed
        ? "Opened the rendered video directly because MP4 conversion was unavailable."
        : "Press and hold the video, then choose Save, to keep it on this device.",
    };
  } catch (err) {
