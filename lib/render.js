...
function pickMimeType(withAudio) {
  ...
  // Android Chrome can produce broken MP4 timing from canvas capture, including
  // a file that effectively contains only the first captured frame. Keep WebM
  // as the reliable recording intermediate on Android; the export layer can
  // convert it to MP4 when needed.
  const preferWebM = android && !ios;
...
export async function recordReel(canvas, state, onProgress) {
  ...
  // Use the browser's continuous canvas capture clock. requestFrame() is useful
  // for controlled capture, but it is not equally reliable across MediaRecorder
  // implementations, especially mobile. Continuous capture is more robust for
  // multi-scene reels because every rendered frame is automatically submitted.
  let stream;
  let videoTrack;
  ...
  try {
    ...
    throw new Error("This browser cannot capture the reel canvas.");
  }
    ...
    setTimeout(() => {
      ...
      function tick() {
        ...
        if (t >= state.total) {
          ...
          frames++;
          // Continuous capture submits this frame automatically.
