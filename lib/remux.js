...
export async function toInstagramMp4(blob, { onProgress, durationSeconds } = {}) {
  ...
  const args = isWebm
    ...
    : [
        "-i", inputName,
        // Copying streams is intentional: the problem is the container,
        // not the encoded frames. +faststart writes a normal moov atom.
        "-map", "0:v:0",
        "-map", "0:a:0?",
        "-c:v", "copy",
        "-c:a", "copy",
  ...
  // allowance for the encoder flush so the final frame/audio is not cut off.
