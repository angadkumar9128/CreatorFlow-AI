...
export async function exportShort({
  ...
  musicMode = "trim", musicVolume = 1, originalVolume = 1,
  creatorHandle = "", handlePosition = "bottom-right", handleOpacity = 0.9, captions = [], onProgress,
}) {
  ...
  try {
    ...
    await new Promise((resolve) => {
      const draw = () => {
        ...
        drawCreatorCaptions(ctx, canvas, captions, start + elapsed, scale);
        drawCreatorHandle(ctx, canvas, creatorHandle, handlePosition, handleOpacity);
    ...
    // Short AI clips can be 8-39s. Some mobile MP4 MediaRecorder outputs
    // keep playable fragment timestamps but leave gallery duration metadata
    // at 00:00. Patch the final MP4 headers to the exact authored duration.
    const finalBlob = await fixMp4Duration(result.blob, Math.round(duration * 1000));
    return finalBlob;
  } finally {
...
function drawCreatorCaptions(ctx, canvas, captions, sourceTime, scale) {
  const caption = (Array.isArray(captions) ? captions : []).find((item) => sourceTime >= Number(item.start) && sourceTime < Number(item.end));
  ...
  const boxW = Math.min(maxWidth + padX * 2, canvas.width - 24 * scale);
  const boxH = lines.length * lineHeight + padY * 2;
  const x = canvas.width / 2;
  const y = canvas.height * 0.76;
  ctx.fillStyle = "rgba(0,0,0,.68)";
  ctx.beginPath();
  if (typeof ctx.roundRect === "function") ctx.roundRect(x - boxW / 2, y - boxH / 2, boxW, boxH, Math.round(12 * scale)); else ctx.rect(x - boxW / 2, y - boxH / 2, boxW, boxH);
  ctx.fill();
  ctx.fillStyle = "#fff";
  lines.forEach((lineText, index) => ctx.fillText(lineText, x, y - ((lines.length - 1) * lineHeight) / 2 + index * lineHeight));
  ctx.restore();
}

function drawCreatorHandle(ctx, canvas, handle, position, opacity) {
  ...
  let x = margin;
  let y = canvas.height - margin - boxH;
  if (position === "bottom-right") x = canvas.width - margin - boxW;
  if (position === "center") { x = (canvas.width - boxW) / 2; y = (canvas.height - boxH) / 2; }
  if (position === "top-left") y = margin;
  if (position === "top-right") { x = canvas.width - margin - boxW; y = margin; }
  ctx.globalAlpha = Math.max(0.35, Math.min(1, Number(opacity) || 0.9));
  ctx.fillStyle = "rgba(0,0,0,.58)";
  ctx.beginPath();
  if (typeof ctx.roundRect === "function") ctx.roundRect(x, y, boxW, boxH, Math.round(10 * scale));
