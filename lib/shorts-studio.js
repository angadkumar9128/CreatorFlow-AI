...
function makeState(base) {
  return {
    ...
    originalVolume: 1, music: null, captions: [], hooks: [], renderStatus: "idle", renderProgress: 0,
    outputUrl: null, renderedBlob: null, selected: false, error: "",
    title: String(base.title || "").trim(), description: String(base.description || "").trim(), hashtags: Array.isArray(base.hashtags) ? base.hashtags : [], aiReason: String(base.aiReason || "").trim(),
  };
}
...
export function createAIShorts(sourceDuration, clips = []) {
  const total = Number(sourceDuration) || 0;
  const aiMin = 8;
  const aiMax = 39;
  return clips
    .map((clip, i) => {
      const start = clamp(Number(clip.start) || 0, 0, Math.max(0, total - aiMin));
      const end = clamp(Number(clip.end) || start + DEFAULT_SHORT_SECONDS, start + aiMin, Math.min(total, start + aiMax));
      if (end - start < aiMin) return null;
      return makeState({
        id: makeId("ai", i + 1),
        index: i + 1,
        sourceStart: start,
        ...
        aiReason: String(clip.reason || "").trim(),
      });
    })
    .filter(Boolean);
}
