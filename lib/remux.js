...
const CORE_ST = `https://unpkg.com/@ffmpeg/core@${CORE_VERSION}/dist/umd`;
const CORE_MT = `https://unpkg.com/@ffmpeg/core-mt@${CORE_VERSION}/dist/umd`;
...
/**
 * Loads the UMD bundle as a real <script> tag, not `import(blobURL)`. The
 * dynamic-import path is what produced "Cannot find module 'blob:...'" —
 * Next's bundler resolves import() specifiers at build time and a runtime
 * blob: URL doesn't fit that model. A classic script tag has no such
 * resolution step; the module attaches itself to `window` and that's it.
 */
  ...
  ffmpegPromise = (async () => {
    ...
    try {
      const [coreURL, wasmURL, workerURL] = await Promise.all([
        ...
        mt ? toBlobURL(`${coreBase}/ffmpeg-core.worker.js`, "text/javascript") : Promise.resolve(undefined),
      ]);
      await ffmpeg.load({ coreURL, wasmURL, workerURL });
    } catch (err) {
      if (!mt) throw err;
      // Multi-thread core failed to fetch or load (e.g. isolation headers
      // present but a CDN hiccup) — retry once on the single-thread core
