...
export async function saveFile(blob, baseName, options = {}) {
  ...
  // Fast path: native MP4 from MediaRecorder is already the Instagram container.
  // Do not invoke FFmpeg for every save — that can take well over a minute in
  // browser WebAssembly. Only WebM/MKV recordings need the conversion fallback.
  if (/webm|matroska/i.test(blob.type || "")) {
  ...
  try {
    ...
    if ("download" in anchor) {
      ...
      return {
        method: "download",
        filename,
        note: "Saved as an MP4 without re-encoding."
      };
    }
    ...
    return {
      method: "opened",
      filename,
      note: "Exported and verified as an Instagram-ready MP4. Press and hold the video, then choose Save, to keep it on this device.",
    };
  } catch (err) {
...
export function instagramCheck({ mimeType, durationSeconds, hasAudio, sizeBytes, verified = false }) {
  const problems = [];
  const warnings = [];

  if (!verified) {
    problems.push("The rendered recording has not been normalized and verified as an Instagram-ready MP4 yet.");
  }
