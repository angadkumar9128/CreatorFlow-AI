...
async function askGeminiYouTubeDirect(youtubeUrl, apiKey, prompt) {
  const key = cleanKey(apiKey);
  const res = await fetch(
    `${GEMINI_API_BASE}/models/${MODEL}:generateContent?key=${encodeURIComponent(key)}`,
    {
  ...
  if (!res.ok) throw new Error(`Gemini YouTube analysis failed (${res.status}): ${(await res.text()).slice(0, 400)}`);
  const data = await res.json();
  const text = data?.candidates?.[0]?.content?.parts?.map((p) => p.text || "").join("") || "";
...
export async function analyzeYouTubeForCreator(youtubeUrl, apiKey, options = {}) {
  ...
  const prompt = `
You are an expert short-form video editor. Analyze the entire public YouTube video at the supplied URL and return ONLY valid JSON.

Goal: identify strong, self-contained moments that could become Instagram Reels / YouTube Shorts / TikToks.

Return up to ${maxClips} non-overlapping clips. Every clip MUST be 8-39 seconds. Do not use a fixed target duration. Choose each duration independently from the actual content and deliberately vary durations when the source supports it. Prefer 15-39 seconds when appropriate, but allow 8-14 seconds for a complete high-value moment. Never add filler to reach a target. Choose natural boundaries around complete thoughts, setup/payoff, useful explanations, punchlines, surprising facts, emotional moments or conclusions.

Also return the approximate total video duration in seconds as "sourceDuration". If exact duration is unavailable, estimate it from the video timeline. Return title, description, hashtags and reason for each clip, plus phrase-level captions and five hooks.
...
Return exactly:
{
  ...
  "clips": [
    {"start": 0, "end": 20, "score": 92, "title": "Short title", "description": "Description", "hashtags": ["#reels"], "reason": "Why it works"}
  ],
  "captions": [{"start": 0, "end": 2.4, "text": "Caption phrase"}],
  "hooks": [{"text": "Hook", "style": "curiosity"}]
}

Rules:
- All times are seconds from the original YouTube video.
- Keep every clip inside 0..sourceDuration.
- HARD LIMIT: every clip is 8-39 seconds.
- No overlap.
- Do not output markdown or commentary.
`;

  options.onProgress?.({ stage: "analyzing", ratio: 0.15 });
  const raw = cleanKey(apiKey)
    ? await askGeminiYouTubeDirect(normalizedUrl, apiKey, prompt)
    : await (async () => {
        const res = await fetch("/api/ai/gemini?action=analyzeYoutube", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ youtubeUrl: normalizedUrl, prompt }),
        });
        const data = await res.json().catch(() => ({}));
        if (!res.ok) throw new Error(data.error || `Gemini YouTube analysis failed (${res.status}).`);
        const text = data?.candidates?.[0]?.content?.parts?.map((p) => p.text || "").join("") || "";
        if (!text) throw new Error("Gemini returned no YouTube analysis.");
        return parseJSON(text);
      })();

  const sourceDuration = Number(raw?.sourceDuration);
  if (!Number.isFinite(sourceDuration) || sourceDuration < 30) throw new Error("Gemini could not determine a valid YouTube video duration of at least 30 seconds.");
  const clips = (Array.isArray(raw.clips) ? raw.clips : [])
    .map((clip) => normalizeClip(clip, sourceDuration))
    .filter(Boolean)
  ...
  return {
    ...
    captions: normalizeCaptions(raw.captions, sourceDuration),
...
Select up to ${maxClips} strong, self-contained short-form moments. HARD REQUIREMENTS: every clip must be at least ${AI_MIN_SHORT_SECONDS} seconds and at most ${AI_MAX_SHORT_SECONDS} seconds. Do NOT follow any fixed target duration. Choose each clip length independently from the actual content, using natural start/end boundaries. Valid lengths include 8-39 seconds; examples are 10s, 15s, 18s, 20s, 25s, 30s, 35s, but these are examples, not targets. Prefer 15-39 seconds when the content supports it, but a shorter 8-14 second clip is valid when that is the complete high-value moment. When selecting multiple clips, deliberately vary their durations when the source supports it; do not make every clip the same length. Never force a clip to reach a target length by adding filler. Never return 2-7 second clips. Choose natural boundaries so the viewer gets a complete thought, setup + payoff, story beat, useful explanation, punchline, surprising fact, emotional moment, or strong conclusion. Do not overlap clips. Prefer hooks, surprising facts, emotional moments, useful explanations, stories, questions, punchlines, strong opinions, or clear conclusions. Avoid long silence, greetings, repeated filler, weak context, and incomplete sentences.
...
- HARD LIMIT: every clip must be 8-39 seconds.
- Do not use a fixed duration target.
- Choose duration from the content, and vary durations across clips when appropriate.
- Prefer 15-39 seconds when the content supports it, but 8-14 seconds is allowed for a complete strong moment.
