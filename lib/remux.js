...
async function loadFFmpegLibrary() {
  ...
  try {
    ...
    if (!source.includes("r.workerLoadURL")) {
      throw new Error("FFmpeg library worker patch did not match the downloaded build.");
    }

    // Do not use import(blobURL) here. Next.js/webpack can treat a runtime
    // blob URL as a module specifier and throw "Cannot find module 'blob:...'",
    // even though the browser itself can execute the UMD build. Load the UMD
    // bundle as a classic script so it registers globalThis.FFmpegWASM.
    ...
    try {
      await new Promise((resolve, reject) => {
        ...
        script.onerror = () => {
          script.remove();
          reject(new Error("Could not execute the FFmpeg browser library."));
        };
...
export async function toInstagramMp4(blob, { onProgress, durationSeconds } = {}) {
  ...
  // Only WebM needs this fallback. Keep it intentionally fast: the recorder
  // already produces 720x1280/24fps on phones, so do not upscale it to 30fps.
  ...
  // ffprobe is not available in every 0.12.x browser build.
  // The encode itself is the source of truth; avoid a second FFmpeg pass.
